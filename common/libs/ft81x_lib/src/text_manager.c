...
ft81x_result ft81x_text_manager_write_fixed_width_font_string(FT81X_Handle *handle, uint8_t font_id, uint8_t font_width, uint8_t font_height, char *str, uint32_t x, uint32_t y)
{
    ...
    res = ft81x_write_display_list_cmd(handle, FT81X_DL_CMD_BEGIN(FT81X_PRIMITIVE_BITMAP));
    ...
    for (int i = 0; str[i]; i++)
    {
        ...
        res = ft81x_write_display_list_cmd(handle, cmd);
        if (res != FT81X_RESULT_OK)
        {
            return res;
        }
    ...
    return ft81x_write_display_list_cmd(handle, FT81X_DL_CMD_END());
}
